        ...
        for channel in channels:
            ...
            try:
                async for message in client_user.iter_messages(int(channel_id), min_id=last_id):
                    # Обновляем max_id для каждого сообщения
                    ...
                    try:
                        ...
                        logger.error(f"Ошибка при пересылке сообщения с ID {message.id} из канала {channel_id}: {e}")

                # Обновление last_message_id после обработки всех сообщений
                if max_id > last_id:
